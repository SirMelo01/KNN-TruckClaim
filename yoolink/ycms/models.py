...
def unique_image_name(instance, filename):
    ...
    return f"knntruck/images/{slugify(base)}_{timestamp}{ext}"
...
def upload_to_galery_image(instance, filename):
    ...
    timestamp = timezone.now().strftime("%Y%m%d%H%M%S")
    base, ext = os.path.splitext(filename)
    return f"knntruck/galeryImages/{slugify(base)}_{timestamp}{ext}"
...
def upload_to_blog_image(instance, filename):
    return f"knntruck/blogs/{instance.id}/{filename}"
def default_code():
    return dict()
...
def upload_to_product_image(instance, filename):
    return f"knntruck/products/{instance.id}/{filename}"
